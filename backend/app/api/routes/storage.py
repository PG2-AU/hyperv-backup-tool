...
def list_luns(db: Session = Depends(get_db), user=Depends(require_permission(Permission.STORAGE_VIEW))) -> list[NetAppLunRead]:
    ...
    return [
        NetAppLunRead(
            ...
            state=l.state, size_bytes=l.size_bytes, used_bytes=l.used_bytes,
            # Kein eigener ONTAP-Prozentwert fuer LUNs (anders als bei Volumes) --
            # dieselbe Formel wie im Kapazitaets-Alarm-Check (scheduler.py).
            percent_used=round(l.used_bytes / l.size_bytes * 100) if l.used_bytes is not None and l.size_bytes else None,
            os_type=l.os_type, mapped_igroups=l.mapped_igroups,
...
def list_aggregates(db: Session = Depends(get_db), user=Depends(require_permission(Permission.STORAGE_VIEW))) -> list[NetAppAggregateRead]:
    ...
    return [
        NetAppAggregateRead(
            ...
            efficiency_ratio=a.efficiency_ratio, efficiency_ratio_wo_snapshots_flexclones=a.efficiency_ratio_wo_snapshots_flexclones,
